        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => set("numeroSecu", e.target.value)}
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary font-mono"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary"
            >
              {Object.entries(MOBILITE_LABEL).map(([v, l]) => (
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary resize-none"
            />
          </div>
...
export default function Patients() {
  ...
  return (
    <div className="p-7 fade-in">
      ...
      {showModal && (
        <ModalNouveauPatient onClose={() => setShowModal(false)} onSuccess={handlePatientCreated} />
      )}
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
              label: "App Mobile",
              val: stats.appMobile || 0,
              icon: "smartphone",
              color: "text-violet-600",
            },
            {
              label: "Fauteuil",
              val: stats.parMobilite?.find((m) => m._id === "FAUTEUIL_ROULANT")?.count || 0,
              icon: "accessible",
              color: "text-blue-600",
            },
      ...
      {loading ? (
        <SkeletonList count={6} of={PatientRowSkeleton} />
      ) : (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                          ...
                          {p.mobilite && p.mobilite !== "ASSIS" && (
                            <span
                              className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${MOBILITE_BADGE_COLOR[p.mobilite] || "bg-slate-100 text-slate-600"}`}
                            >
            ...
            {!patientSelectionne ? (
              <div className="bg-white rounded-xl border border-slate-200 p-16 text-center h-full flex flex-col items-center justify-center">
                ...
                <p className="text-slate-400 text-sm mt-3">
                  Sélectionnez un patient pour voir sa fiche
                </p>
              </div>
            ) : (
              <PatientDetail
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {p.dateNaissance && (
                <span className="text-xs text-slate-500 flex items-center gap-1">
                  ...
                  {fmtDate(p.dateNaissance)}
                </span>
              )}
              {p.mobilite && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${MOBILITE_BADGE_COLOR[p.mobilite] || "bg-slate-100 text-slate-600"}`}
                >
                  {MOBILITE_LABEL[p.mobilite]}
                </span>
              )}
        ...
        {(p.oxygene || p.brancardage || p.accompagnateur) && (
          <div className="flex gap-2 flex-wrap mb-3">
            {p.oxygene && (
              <span className="text-xs bg-cyan-50 text-cyan-700 border border-cyan-200 px-2 py-1 rounded-full">
                Oxygène
              </span>
            )}
            {p.brancardage && (
            ...
            {p.accompagnateur && (
              <span className="text-xs bg-violet-50 text-violet-700 border border-violet-200 px-2 py-1 rounded-full">
                Accompagnateur
              </span>
            )}
          </div>
          ...
          {p.contactUrgence?.nom && (
            <div>
              ...
              <p className="text-slate-700 mt-0.5">
                {p.contactUrgence.nom} ({p.contactUrgence.lien}) — {p.contactUrgence.telephone}
              </p>
            </div>
          )}
        </div>
