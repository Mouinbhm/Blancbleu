...
export default function Layout() {
  ...
  useEffect(() => {
    return subscribe("transport:created", (data) => {
      ...
      setToasts((prev) =>
        [{ id: toastId, data, path: `/transports/${String(data._id)}` }, ...prev].slice(0, 3),
      );
  ...
  useEffect(() => {
    return subscribe("transport:assigned", (data) => {
      addLocalNotif({
        ...
        message: data.numero
          ? `Transport ${data.numero} — véhicule affecté`
          : "Un véhicule a été assigné",
  ...
  useEffect(() => {
    return subscribe("transport:late", (data) => {
      addLocalNotif({
        ...
        message:
          data.message ||
          (data.numero ? `Transport ${data.numero} en retard` : "Un transport est en retard"),
      });
    });
  }, [subscribe, addLocalNotif]);
      ...
      addLocalNotif({
        ...
        message:
          ...
          (data.immatriculation
            ...
            : "Un véhicule nécessite une maintenance"),
      });
    });
  }, [subscribe, addLocalNotif]);
  ...
  useEffect(() => {
    const unsubStart = subscribe("shift:started", (data) => {
      addLocalNotif({
        type: "shift:started",
        title: "Shift démarré",
        message: data.driverNom
          ? `${data.driverNom} a démarré son shift`
          : "Un shift vient de commencer",
      });
    });
    const unsubEnd = subscribe("shift:ended", (data) => {
      addLocalNotif({
        ...
        title: "Shift terminé",
        message: data.driverNom
          ? `${data.driverNom} a terminé son shift`
          : "Un shift vient de se terminer",
      });
    });
    return () => {
      unsubStart();
      ...
      <aside className="w-60 h-screen fixed left-0 top-0 bg-navy flex flex-col z-50 shadow-xl">
        ...
        <div className="px-4 py-3 border-t border-white/5">
          <div className="flex items-center gap-2 px-1 mb-2">
            <span className="material-symbols-outlined text-slate-600" style={{ fontSize: "13px" }}>
              location_on
            </span>
            <span
                      ...
                      allNotifs.map((n) => {
                        ...
                        return (
                          ...
                          >
                            {/* Icône colorée */}
                            <div
                              className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${palette.bg}`}
                            >
                              <span
                                className={`material-symbols-outlined text-base ${palette.icon}`}
                              >
                                {meta.icon}
                              </span>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={`text-sm leading-snug ${!n.read ? "font-semibold text-navy" : "font-medium text-slate-700"}`}
                              >
                                {n.title}
                              </p>
                              {n.message && (
                              ...
                              <p
                                className={`text-xs mt-1 font-medium ${!n.read ? "text-primary" : "text-slate-400"}`}
                              >
                                {timeAgo(n.time)}
                              </p>
                            </div>
                            ...
                            {!n.read && (
                              <span
                                className={`w-2.5 h-2.5 rounded-full flex-shrink-0 mt-2 ${palette.dot}`}
                              />
                            )}
                          </div>
                        );
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3 p-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${palette.bg}`}
                >
                  <span
                ...
                <button
                  onClick={() => dismissNotifToast(toast.id)}
        ...
        >
          ...
          >
            <div className="bg-red-600 px-6 py-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-mono text-xs tracking-widest font-bold">
                  ALERTE URGENTE
                </p>
