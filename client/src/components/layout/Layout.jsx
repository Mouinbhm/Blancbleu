...
export default function Layout() {
  ...
  // ── Socket temps réel ─────────────────────────────────────────────────
  const { connected, nouvellesInterventions, alertesP1 } = useSocket();

  // Fusionner les nouvelles interventions socket avec les notifs existantes
  useEffect(() => {
    if (nouvellesInterventions.length > 0) {
      const latest = nouvellesInterventions[0];
      const socketNotif = {
        id:    latest._id + '_socket',
        title: `${latest.priorite} — ${latest.typeIncident}`,
        sub:   latest.statut === 'en_attente' ? "En attente d'une unité" : "Intervention créée",
        color: latest.priorite === 'P1' ? "text-danger" : latest.priorite === 'P2' ? "text-warning" : "text-primary",
        path:  "/interventions",
        time:  new Date(latest.createdAt || Date.now()),
      };
      setNotifs(prev => [socketNotif, ...prev].slice(0, 8));
      setNotifCount(prev => prev + 1);
    }
  }, [nouvellesInterventions]);

  // Alerte P1 → badge rouge clignotant
  useEffect(() => {
    if (alertesP1.length > 0) {
      setNotifCount(prev => prev + 1);
    }
  }, [alertesP1]);
  ...
  useEffect(() => {
    const loadNotifs = async () => {
      try {
        const { data } = await interventionService.getAll({ statut: "en_attente", limit: 10 });
        const enAttente = data.interventions || [];
        const { data: data2 } = await interventionService.getAll({ statut: "en_cours", limit: 5 });
        ...
        const notifList = [
          ...enAttente.map(i => ({
            ...
            color: i.priorite === "P1" ? "text-danger" : i.priorite === "P2" ? "text-warning" : "text-primary",
            path:  "/interventions",
        ...
        <div className="px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-primary flex items-center justify-center flex-shrink-0">
              <span className="material-symbols-outlined text-white" style={{fontSize:'18px'}}>emergency</span>
            </div>
            <div>
            ...
            <NavLink key={item.path} to={item.path}
              className={({ isActive }) =>
                `flex items-center justify-between px-4 py-2.5 rounded-lg transition-all text-sm font-medium ${
                  isActive ? "bg-primary text-white shadow-lg shadow-primary/30" : "text-slate-400 hover:text-white hover:bg-white/5"
                }`
              }
            >
              <div className="flex items-center gap-3">
              ...
              {item.path === "/interventions" && notifCount > 0 && (
                <span className="bg-danger text-white text-xs font-mono font-bold px-1.5 py-0.5 rounded-full">{notifCount}</span>
              )}
            </NavLink>
          ))}
        ...
        <div className="px-4 pb-4 border-t border-white/10 pt-3 space-y-2">
          <div className="flex items-center gap-3 bg-white/5 rounded-xl p-3 border border-white/10">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-yellow-400 font-mono" style={{fontSize:'11px'}}>SHIFT: {shiftTime}</p>
            </div>
            <button onClick={logout} title="Déconnexion" className="text-slate-500 hover:text-red-400 transition-colors">
              <span className="material-symbols-outlined" style={{fontSize:'18px'}}>logout</span>
            </button>
          </div>
          <div className="flex items-center gap-2 px-1">
            <span className="w-2 h-2 rounded-full bg-success animate-pulse flex-shrink-0" />
            <span className="text-slate-600 font-mono" style={{fontSize:'9px', letterSpacing:'0.1em'}}>SYSTÈME OPÉRATIONNEL</span>
          </div>
        </div>
      </aside>
              ...
              {notifOpen && (
                <div className="absolute right-0 top-11 w-80 bg-white rounded-xl shadow-xl border border-slate-200 z-50">
                  <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    {notifCount > 0 && (
                      <span className="bg-danger text-white text-xs font-bold px-2 py-0.5 rounded-full">{notifCount} en attente</span>
                    )}
                  </div>
                  {notifs.length === 0 ? (
                  ...
                  <div className="p-3 border-t border-slate-100">
                    <button onClick={() => { navigate("/interventions"); setNotifOpen(false); }}
