...
export const comptabiliteKeys = {
  all: ["comptabilite"],
  dashboard: (annee, mois) => ["comptabilite", "dashboard", annee, mois],
};
...
export function useFactureStats() {
  return useQuery({
    ...
    queryFn: () => factureService.getStats().then((r) => r.data),
  });
}

export function useComptabiliteDashboard(annee, mois) {
  return useQuery({
    queryKey: comptabiliteKeys.dashboard(annee, mois),
    queryFn: () => comptabiliteService.getDashboard({ annee, mois }).then((r) => r.data),
  });
}
...
export function useFactureMutation() {
  ...
  return {
    create: useMutation({
      mutationFn: (d) => factureService.create(d),
    ...
    issue: useMutation({
      mutationFn: (id) => factureService.issue(id),
      onSuccess: (_, id) => invalidate(id),
    }),
    refund: useMutation({
      mutationFn: ({ id, amount, reason }) => factureService.refund(id, amount, reason),
      onSuccess: (_, { id }) => invalidate(id),
    }),
    updateStatut: useMutation({
      mutationFn: ({ id, statut }) => factureService.updateStatut(id, statut),
      onSuccess: (_, { id }) => invalidate(id),
    }),
    remove: useMutation({
