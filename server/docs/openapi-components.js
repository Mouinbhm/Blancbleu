...
const securitySchemes = {
  ...
  cookieAuth: {
    ...
    in:   "cookie",
    name: "bb_access",
    description: "Cookie httpOnly émis par /api/auth/login. Utilisé en priorité par les clients browser.",
  },
  serviceTokenAuth: {
    type: "apiKey",
    in:   "header",
    name: "X-Service-Token",
    description: "Token partagé Node ↔ Python pour les routes service-to-service (training-data, model/retrain, model/status).",
  },
};
...
const schemas = {
  ...
  User: {
    ...
    properties: {
      ...
      role:                    { type: "string", enum: ["dispatcher", "superviseur", "admin", "comptable", "patient"] },
    ...
    properties: {
      ...
      statut: {
        ...
        enum: [
          ...
          "CANCELLED", "NO_SHOW", "RESCHEDULED", "FAILED",
        ],
      },
  ...
  Facture: {
    ...
    properties: {
      ...
      statut: {
        type: "string",
        enum: ["brouillon", "emise", "en_attente", "payee", "annulee", "payment_failed", "remboursee", "partiellement_remboursee", "en_retard"],
      },
      paymentStatus: {
        type: "string",
