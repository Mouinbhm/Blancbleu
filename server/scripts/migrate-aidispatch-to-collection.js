...
async function run() {
  ...
  for await (const t of cursor) {
    ...
    if (t.aiDispatch?.lastRecommendationId) {
      const exists = await DispatchRecommendation.exists({
        _id: t.aiDispatch.lastRecommendationId,
      });
      if (exists) {
        ignorés++;
        continue;
    ...
    const rec = await DispatchRecommendation.create({
      ...
      decision: {
        ...
        rejectionReason:
          decisionStatus === "rejected" ? t.aiDispatch.rejectedReason || "" : undefined,
    ...
    await Transport.updateOne(
      { _id: t._id },
      { $set: { "aiDispatch.lastRecommendationId": rec._id } },
    );
    créés++;
